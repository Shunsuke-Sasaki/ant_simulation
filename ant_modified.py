...
class Ant(pg.sprite.Sprite):
    ...
    def __init__(self, drawSurf, nest, pheroLayer):
        ...
        self.threshold = randint(1, 10)  # 1から10のランダムな閾値を設定
        self.efficiency = 10  # 仮の初期効率を設定
    ...
    def update(self, dt):  # behavior
        ...
        if self.mode == 0 and self.pos.distance_to(self.nest) > 21 and self.threshold <= 3:
            self.mode = 1

        # アリが閾値以上の場合、餌集めの効率が下がったら餌を運ぶ行動を実行
        elif self.mode == 0 and self.pos.distance_to(self.nest) > 21 and self.threshold > 3:
            efficiency_threshold = 10-0.2*self.food_collected  # 仮の効率の閾値
            if self.threshold < efficiency_threshold:
                self.mode = 1
...
def main():
    ...
    while True:
        c+=1
        if(c==10000):
            with open('food_count_modified.csv', 'w') as file:
                file.write(str(Ant.food_collected_total))
                file.write('feas')
            break
